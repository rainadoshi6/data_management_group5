...
-- Next I build up the logic I used to parse the data in the original text
-- First I would like to extract the event_id. I note that it is the only text in parentheses
...
-- That's pretty good but it includes the parentheses. 
-- I got rid of the right one using the substring function 
-- and trim to get rid of the right one
SELECT 
...
-- ---------------------------------------------------------------------
-- 		A FEW HINTS BELOW TO HELP YOU WITH THE HOMEWORK
--      You will have to use the MySQL documentation to complete the rest
-- ---------------------------------------------------------------------
...
-- We extracted the date but in text form 
-- The STR_TO_DATE function can convert strings to dates (and times)
...
-- This version specifies how to convert the date from the format used in the original_text
SELECT STR_TO_DATE("4/14/17" , "%m/%d/%y");

-- Refer to the MySQL documentation to figure out how to extract the time as well
...
-- The following UPDATE query extracts the data from the original_text field and updates the corresponding
-- fields in the table. The call_date field is converted to the DATE data type using the STR_TO_DATE function. 
-- Note that I had to cast the results from regexp_substr to CHAR type before using STR_TO_DATE. 

-- You should submit this UPDATE querty with the addition of code to parse the original_text field to extract
-- the call_time, event_address, and resp_agency (responding agency) and store the results in the corresponding fields
UPDATE calls_911
SET 
